...
void MK66F18_clock_init(void) {
  ...
  WDOG->UNLOCK = 0xC520;
  WDOG->UNLOCK = 0xD928;
  WDOG->STCTRLH &= ~WDOG_STCTRLH_WDOGEN(1);
  ...
  OSC->CR = KINETIS_BOARD_OSCILLATOR_SETTING;
  ...
  /* Switch to crystal as clock source*/
  MCG->C1 = MCG_C1_CLKS(2) | MCG_C1_FRDIV(KINETIS_C1_FRDIV);
...
#endif
                 SIM_CLKDIV1_OUTDIV4(KINETIS_CLKDIV1_OUTDIV4-1);
  SIM->CLKDIV2 &= ~(SIM_CLKDIV2_USBFRAC_MASK | SIM_CLKDIV2_USBDIV_MASK);
  SIM->CLKDIV2 |= (SIM_CLKDIV2_USBFRAC(KINETIS_CLKDIV2_USBFRAC) |
                  SIM_CLKDIV2_USBDIV(KINETIS_CLKDIV2_USBDIV));
