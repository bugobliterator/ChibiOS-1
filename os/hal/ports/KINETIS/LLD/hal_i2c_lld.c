...
static void i2c_lld_serve_tx_end_irq(I2CDriver* i2cp) {
  _disable_i2c_dma(i2cp);
  if (i2cp->intstate == STATE_SEND) {
    i2cp->i2c->C1 &= ~I2Cx_C1_DMAEN;
    i2cp->txidx = i2cp->txbytes;
  } else if(i2cp->intstate == STATE_RECV) {
    i2cp->i2c->C1 &= ~I2Cx_C1_DMAEN;
    i2c->C1 |= I2Cx_C1_TXAK;
    i2cp->rxidx = i2cp->rxbytes - 1;
  }
}
...
#if KINETIS_I2C_USE_DMA == TRUE
            _enable_i2c_dma(i2cp);
#else
...
void i2c_lld_init(void) {
  ...
  I2CD1.i2c = I2C0;
#if KINETIS_I2C_USE_DMA == TRUE
  I2CD1.rxdma = KINETIS_DMA_CHANNEL(KINETIS_I2C_I2C1_RX_DMA_CHANNEL);
  I2CD1.txdma = KINETIS_DMA_CHANNEL(KINETIS_I2C_I2C1_TX_DMA_CHANNEL);
#endif
#endif
...
#if KINETIS_I2C_USE_DMA == TRUE
  I2CD2.rxdma = KINETIS_DMA_CHANNEL(KINETIS_I2C_I2C2_RX_DMA_CHANNEL);
  I2CD2.txdma = KINETIS_DMA_CHANNEL(KINETIS_I2C_I2C2_TX_DMA_CHANNEL);
#endif
   ...
   */
    ...
    if (&I2CD1 == i2cp) {
...
#if KINETIS_I2C_USE_DMA == TRUE
      {
        ...
        b = dmaChannelAllocate(i2cp->dma,
                              KINETIS_I2C_I2C1_IRQ_PRIORITY,
                              (kinetis_edmaisr_t)i2c_lld_serve_end_irq,
                              (void *)i2cp);
        dmaChannelSetSource(i2cp->dmarx, kDmaRequestMux0I2C0);
        osalDbgAssert(!b, "stream already allocated");
      }
#endif /* KINETIS_I2C_USE_DMA == TRUE */
    ...
    if (&I2CD2 == i2cp) {
...
#if KINETIS_I2C_USE_DMA == TRUE
      {
        ...
        b = dmaChannelAllocate(i2cp->dmarx,
                              KINETIS_I2C_I2C2_IRQ_PRIORITY,
                              (kinetis_edmaisr_t)i2c_lld_serve_end_irq,
                              (void *)i2cp);
          dmaChannelSetSource(i2cp->dmarx, kDmaRequestMux0I2C1);
        osalDbgAssert(!b, "stream already allocated");
      }
#endif /* KINETIS_I2C_USE_DMA == TRUE */
...
static inline void _enable_i2c_dma(I2CDriver *i2cp) {
  if (i2cp->intstate == STATE_SEND) {
    dmaChannelSetSourceAddr(i2cp->dma, txbuf, 1);
    dmaChannelSetSourceAddrAdj(i2cp->dma, 0);
    dmaChannelSetDestinyAddr(i2cp->dma, &i2cp->i2c->D, 0);
    dmaChannelSetDestinyAddrAdj(i2cp->dma, 0);
    dmaChannelSetNumBytesPerTransaction(i2cp->dma, 1);
    dmaChannelSetTransactionSize(i2cp->dma, txbytes);
    dmaChannelSetAttr(i2cp->dma, (DMA_ATTR_SSIZE(0) | DMA_ATTR_DSIZE(0)));
    i2cp->dma->channel->CSR = DMA_CSR_INTMAJOR | DMA_CSR_DREQ;
    dmaChannelEnable(i2cp->dma);
    i2cp->i2c->C1 |= I2Cx_C1_DMAEN;
  } else if (i2cp->intstate == STATE_RECV) {
    dmaChannelSetSourceAddr(i2cp->dma, &i2cp->i2c->D, 0);
    dmaChannelSetSourceAddrAdj(i2cp->dma, 0);
    dmaChannelSetDestinyAddr(i2cp->dma, rxbuf, 1);
    dmaChannelSetDestinyAddrAdj(i2cp->dma, 0);
    dmaChannelSetNumBytesPerTransaction(i2cp->dma, 1);
    dmaChannelSetTransactionSize(i2cp->dma, rxbytes - 1);
    dmaChannelSetAttr(i2cp->dma, (DMA_ATTR_SSIZE(0) | DMA_ATTR_DSIZE(0)));
    i2cp->dma->channel->CSR = DMA_CSR_INTMAJOR | DMA_CSR_DREQ;
    dmaChannelEnable(i2cp->dma);
    i2cp->i2c->C1 |= I2Cx_C1_DMAEN;
...
static inline void _disable_i2c_dma(I2CDriver *i2cp) {
  dmaChannelDisable(i2cp->dma);
}
...
#if KINETIS_I2C_USE_DMA == TRUE
  if (i2cp->rxbytes > 1 && i2cp->intstate == STATE_RECV) {
    _enable_i2c_dma(i2cp);
  }
#endif
  /* send slave address */
  i2cp->i2c->D = i2cp->addr << 1 | op;
